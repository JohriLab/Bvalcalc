...
def main():
    ...
    if known_args.genome: # Run genome Bcalc
        ...
        if getattr(args, 'plot', True):
            ...
            plotB(b_values_input=output_data, caller="chromosome", output_path=args.plot, quiet=args.quiet, gene_ranges=block_ranges, neutral_only=args.neutral_only, rec_rates=rec_rate_per_chunk_in_region, chunk_size=chunk_size, no_title=getattr(args, 'no_title', False))
